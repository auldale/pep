...
Application::Application(const PEP* pep) :
		m_PEP(pep), m_lock(RWLock::create()) {
...
const PEP& Application::getPEP() const {
	return *m_PEP;
}
...
void Application::clearHeader(PEPRequest& request, const char* rawname, const char* cginame) const {
	request.clearHeader(rawname, cginame);
}

void Application::setHeader(PEPRequest& request, const char* name, const char* value) const {
	request.setHeader(name, value);
}

string Application::getSecureHeader(const PEPRequest& request, const char* name) const {
	return request.getSecureHeader(name);
}

void Application::clearAttributeHeaders(PEPRequest& request) const {
	if (PEPConfig::getConfig().isEnabled(PEPConfig::OutOfProcess)) {
		for_each(m_unsetHeaders.begin(), m_unsetHeaders.end(),
				boost::bind(&PEPRequest::clearHeader, boost::ref(request),
	...
	if (m_unsetHeaders.empty()) {
		...
		if (m_unsetHeaders.empty()) {
			...
			out = getPEP().getListenerService()->send(in);
			if (out.islist()) {
