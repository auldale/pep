...
SocketListener::SocketListener(const DOMElement* e) :
		m_catchAll(false), log(&Category::getInstance(SHIBSP_LOGCAT".Listener")), m_shutdown(
				nullptr), m_stackSize(0), m_socket((ShibSocket) 0) {
	// Are we a client?

	if (pep::PEPConfig::getConfig().isEnabled(pep::PEPConfig::InProcess)) {
		m_socketpool.reset(new SocketPool(*log, this));
	}
	// Are we a server?
	if (pep::PEPConfig::getConfig().isEnabled(pep::PEPConfig::OutOfProcess)) {
	...
	while (!*m_shutdown) {
		...
		default: {
			...
			try {
				...
				log->crit("exception starting new server thread to service incoming request: %s",
						ex.what());
			} catch (...) {
...
//                throw ListenerException("Failure sending remoted message ($1).", params(1,in.name()));
				throw ListenerException("Failure sending remoted message ($1).");
		} else {
			// SUCCESS.
			retry = -1;
		}
	...
	while (recv(sock, (char*) &len, sizeof(len)) != sizeof(len)) {
		if (errno == EINTR)
			continue;  // Apparently this happens when a signal interrupts the blocking call.
		log->error("error reading size of output message");
		this->close(sock);
		throw ListenerException("Failure receiving response to remoted message ($1).");
	...
	if (out.isstring() && out.name() && !strcmp(out.name(), "exception")) {
		...
		try {
			...
			log->error("caught XMLToolingException while building the XMLToolingException: %s",
					e.what());
...
bool SocketListener::log_error(const char* fn) const {
	...
	log->error("socket call (%s) resulted in error (%d): %s", fn, rc,
			isprint(*buf) ? buf : "no message");
#endif
	return false;
}
...
ServerThread::ServerThread(SocketListener::ShibSocket& s, SocketListener* listener,
		unsigned long id) :
	...
	cerr << "stacksize"<< m_listener->m_stackSize<<endl;
...
void ServerThread::run() {
	...
	while (!*(m_listener->m_shutdown)) {
		...
		case -1:
			...
			m_listener->log->error("select() on incoming request socket (%u) returned error",
					m_sock);
			return;
